	...
	function pdf_builder_scripts($hook){
        ...
        if ( in_array( $hook, array( 'post-new.php', 'post.php' ) ) ){
            if( (isset($_GET['post_type']) && ($_GET['post_type'] == 'certificate')) || (isset($_GET['post']) && (get_post_type($_GET['post']) == 'certificate'))){//lkj//
                wp_enqueue_style( 'pdf_builder_css', plugins_url( '../assets/css/pdf_builder.css' , __FILE__ ),array(),WPLMS_PDF_VERSION);
                
                wp_enqueue_script( 'pdf_builder_script', plugins_url( '../assets/js/pdf_builder.js' , __FILE__ ),array('jquery','jquery-migrate','jquery-ui-core','jquery-ui-draggable','jquery-ui-slider'),WPLMS_PDF_VERSION);
