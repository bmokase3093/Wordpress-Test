...
class Vibe_Bbb_Settings{
    ...
    public static function init(){
        ...
        if ( is_null( self::$instance ) )
            self::$instance = new Vibe_Bbb_Settings();
        return self::$instance;
    }

	private function __construct(){
		if($_GET['tab'] == 'vibe_bbb_function'){
			$this->settings=get_option('vibe_bbb_settings');
        }
	...
	function initialize(){
		if(class_exists('VibeBP_Settings')){
			add_filter('vibebp_settings_tabs',function($tabs){
				$tabs['vibe_bbb_function']=__('BBB ','vibe-bbb');
				return $tabs;
			});
		}else{
			add_options_page(__('BBB ','vibe_bbb'),__('BBB ','vibe-bbb'),'manage_options','vibe_bbb',array($this,'settings'),1011);	
		}
		
    }
    
	function get_settings(){
		return apply_filters('vibe_bbb_settings',array(
			array(
				'label' => __('Enable meeting reminder','vibe-bbb'),
				'name' => 'vibe_bbb_enable_reminder',
				'type' => 'checkbox',
				'default'=> 0,
			),
			array(
				'label' => __('Meeting reminder time','vibe-bbb'),
				'name' => 'vibe_bbb_reminder_time',
				'type' => 'select',
				'options' => bbb_reminder_options(),
				'default' => '3600',
			),
	...
	function settings(){
		...
		wp_nonce_field('vibe_bbb_settings');   
		...
		echo '<tr valign="top"><th colspan="2"><input type="submit" name="save_vibe_bbb_settings" class="button button-primary" value="'.__('Save Settings','vibe-bbb').'" /></th>';
			...
			switch($setting['type']){
				...
				case 'file':
						        ...
						        var selection = media_uploader2.state().get('selection');
						            ...
						            if(attachment &&  attachment.url !== undefined ){
						               pdf_url=attachment.url;
						            	...
						            	alert('<?php echo _x("Unable to find url of selected file","","vibe_bbb");?>');
						            }
					...
					echo '<td class="forminp">
							<a class="upload_pdf_button button" data-uploader-button-text="'.__('Upload file','vibe-bbb').'" data-input-name="'.$setting['name'].'" >'.__('Upload','vibe-bbb').'</a>
				...
				case 'repeatable':
					...
					echo '<a class="wa_meta_box_repeatable_add button button-primary button-large" href="#">'.__('Add More','vibe-bbb').'</a>
				...
				case 'page':
					echo '<th scope="row" class="titledesc"><label>'.$setting['label'].'</label></th>';
					echo '<td class="forminp"><select name="'.$setting['name'].'"><option>'.__('Select a page','vibe-bbb').'</option>';
				...
				case 'instructor_field':
					...
					echo '<td class="forminp">';
					?>
					<a id="add_instructor_block" class="button"><?php _e('Add Searchable Instructor Field','vibe-bbb'); ?></a>
						...
						jQuery(document).ready(function($){
							...
							var set_instructor_field = function(element){
								...
								element.find('.set_field').on('change',function(){
									...
									if(i === 'new' ){
										$(this).after('<div class="newoption"><input type="text" placeholder="<?php _e('Value','vibe-bbb'); ?>" class="field_value"><input type="text" class="field_label " placeholder="<?php _e('Label','vibe-bbb'); ?>"><a class="add_newoption button"><?php _e('Add','vibe-bbb'); ?></a></div>');
							...
							if(instructor_blocks.length){
								instructor_blocks.map(function(item,i){
									if(item && item.hasOwnProperty('label')){
										...
										let block = '<li data-i="'+i+'"><span class="remove"></span><select class="set_field"><option value="none">'+item.label+'</option><option value="new"><?php _e('Add new option','vibe-bbb'); ?></option>';
							...
							jQuery('#add_instructor_block').on('click',function(){
								...
								let appender = '<li data-i="'+instructor_blocks.length+'"><div style="display: grid; padding: 10px; text-align: center; grid-gap: 5px;"><input type="text" class="field_key" placeholder="<?php _e('Set Field key','vibe-bbb'); ?>"><input type="text" class="field_name" placeholder="<?php _e('Set Field label','vibe-bbb'); ?>"><a class="add_field_label button-primary"><?php _e('Add Field','vibe-bbb'); ?></a></div><span class="remove"></span></li>';
								...
								jQuery('.add_field_label').on('click',function(){
									...
									if($(this).parent().find('input.field_name').val().length){
										...
										$(this).parent().after('<select class="set_field"><option value="none">'+$(this).parent().find('input.field_name').val()+'</option><option value="new"><?php _e('Add new option','vibe-bbb'); ?></option></select>');
				...
				case 'directory_search':
					...
					echo '<td class="forminp">';
					?>
					<a id="add_search_block" class="button"><?php _e('Add Search block','vibe-bbb'); ?></a>
					<ul id="search_blocks">
					</ul>
					<script>
						...
						jQuery(document).ready(function($){
							...
							$filters = array(
								array(
									'type'=>'course',
									'label'=> __('Search Course','vibe-bbb'),
								),
								array(
									'type'=>'availability',
									'label'=> __('Select Times','vibe-bbb'),
								),
								array(
									'type'=>'course-cat',
									'label'=> __('Select Category','vibe-bbb'),
								),
								array(
									'type'=>'location',
									'label'=> __('Select Location','vibe-bbb'),
								),
								array(
									'type'=>'level',
									'label'=> __('Select Levels','vibe-bbb'),
								),
								array(
									'type'=>'text',
									'label'=> __('Select Text','vibe-bbb'),
								),
							);
							...
							jQuery('#add_search_block').on('click',function(){
								...
								let options ='<option><?php _e('Select search option','vibe-bbb'); ?></option>';
	...
	function save(){
		...
		if(!isset($_POST['save_vibe_bbb_settings']))
			return;
		

		if ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],'vibe_bbb_settings') ){
		     echo '<div class="error notice is-dismissible"><p>'.__('Security check Failed. Contact Administrator !','vibe-bbb').'</p></div>';
		}
		...
		if(!empty($_POST['vibe_bbb_enable_reminder']) && !empty($user_id)){
			$init = Vibe_Bbb_Init::init();
			$init->new_bp_core_email_register($user_id); //new email insert	
		}
		
		
		update_option('vibe_bbb_settings',$this->settings);
		echo '<div class="updated notice is-dismissible"><p>'.__('Settings Saved.','vibe-bbb').'</p></div>';
	}

}

Vibe_Bbb_Settings::init();


function vibe_bbb_function(){
	$init = Vibe_Bbb_Settings::init();
	$init->settings();
}
