...
class VibeBP_Init{
	...
	function get_profile_photo_object(){
		...
		return array(
				'name'            => _x( 'Change Profile Photo', 'Profile header sub menu', 'wplms' ),
				'slug'            => 'change-avatar',
				'parent_slug'     => $slug,
...
function vibebp_get_setting($field,$type='general',$sub=null){
	$init = VibeBP_Init::init();
	return apply_filters('vibebp_get_setting',$init->get_setting($field,$type,$sub),$field);
}
...
function vibebp_get_pwa_styles($force=null){
	$vibe_styles = array(
	    	'bp-member-block','bp-group-block','vibebp-frontend','vibecal','vibeforms_profile_css','tabulator','helpdesk','vibekb','vicons','vibe-projects','vibedrive_profile_css','wplms-cc', 'wplms_dashboard_css', 'vibebp_main', 'vibebp_profile_libs', 'plyr', 'vibe_editor','swiper');
