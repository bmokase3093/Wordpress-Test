...
class VibeBP_Actions{
	...
	function add_bp_nav(){
	    ...
	    bp_core_new_subnav_item( array(
			...
			'slug' => 'balance', 
			'parent_slug' => 'wallet',
        	'parent_url' => $bp->displayed_user->domain.'wallet/balance/',
			'screen_function' => array($this,'show_screen'),
			'user_has_access' => (bp_is_my_profile() || current_user_can('manage_options'))
		) );
		bp_core_new_subnav_item( array(
			...
			'slug' => 'transactions', 
			'parent_slug' => 'wallet',
        	'parent_url' => $bp->displayed_user->domain.'wallet/transactions',
			'screen_function' => array($this,'show_screen'),
			'user_has_access' => (bp_is_my_profile() || current_user_can('manage_options'))
		) );
    ...
    function generate_manifest($force=0){
    	...
    	$app_splash = vibebp_get_setting('app_screenshot','service_worker');
    	...
    	if(empty($background_color)){$background_color = '#fafafa';}
   		...
   		$display_mode = vibebp_get_setting('app_display','service_worker');
...
;		?>
    ...
    function show_google_captcha(){
    	...
    	$google_captcha_public_key = vibebp_get_setting('google_captcha_private_key');
    ...
    function google_captcha_validate(){
    	...
    	$google_captcha_private_key = vibebp_get_setting('google_captcha_private_key');
