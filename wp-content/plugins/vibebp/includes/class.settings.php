...
class VibeBP_Settings{
	...
	function vibebp_settings_tabs($tab){
		$tabs = apply_filters('vibebp_settings_tabs',array( 
	    		...
	    		'registration_forms' => __('Registration Forms','vibebp'), 
	...
	function vibebp_cards(){
		...
		switch($sub){
			...
			case 'add':
				if(!empty($_POST)){
					if ( isset($_POST['add_card'])  && wp_verify_nonce(sanitize_text_field($_POST['add_card']),'add_card') ){
				        $cards[]=wp_kses_post($_POST['card']);
			...
			break;
			default:
				if(in_array(sanitize_text_field($_GET['sub']),array_keys($post_types))){
	...
	function vibebp_general_settings(){
		...
		switch($_GET['sub']){
			...
			$settings = apply_filters('vibebp_general_settings',array(
					...
					array(
						'label' => __('Instructor content privacy','vibebp'),
						'name' => 'instructor_privacy',
						'type' => 'checkbox',
						'desc' => __('Will not show instructors each other content like media etc. ','vibebp'),
						'default'=>''
					),
					array(
		...
		foreach($settings as $setting ){
			...
			switch($setting['type']){
				...
				case 'touchpoint': 
					...
					echo '&nbsp;&nbsp;'.sprintf(__('%s Edit Email Template %s','vibebp'),'<a href="'.$setting['value']['student'].'" class="button">','</a>');
					echo '</td></tr>';
					if(!empty($setting['value']['instructor'])){
						...
						echo '&nbsp;&nbsp;'.sprintf(__('%s Edit Email Template %s','vibebp'),'<a href="'.$setting['value']['instructor'].'" class="button">','</a>');
						echo '</td>
				...
				case 'touchpoint_admin': 
					...
					echo '</select>';
					echo '&nbsp;&nbsp;'.sprintf(__('%s Edit Email Template %s','vibebp'),'<a href="'.$setting['value']['admin'].'" class="button">','</a>');
					echo '</td>
						<tr><td colspan="3"><hr></td>';
				break;
				...
				if(!empty($_POST['google_play_app_products']) && !empty($_POST['google_play_app_products']['product'])){
					...
					foreach ($_POST['google_play_app_products']['product'] as $key => $product_id) {
						if(!empty($product_id) && !empty($_POST['google_play_app_products']['credits']) && !empty($_POST['google_play_app_products']['credits'][$key])){
							$val[] = array('product'=>$product_id,'credits'=>$_POST['google_play_app_products']['credits'][$key]);
						}
					}
					$_POST['google_play_app_products'] = $val;
				}
				if(!empty($_GET['sub'])){
					$vibebp_settings[$tab][sanitize_text_field($_GET['sub'])] = apply_filters('vibebp_save_settings',$_POST,$tab);  
				}else{
	...
	function registration_forms(){
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				$('#wplms_user_bp_group').select2({
			      allowClear: true,
			      placeholder: "<?php echo _x('Select groups setting ','','vibebp');?>"
			    });
		...
		*/
		...
		foreach($groups as $group){
			echo '<h4>'._x('Field Group','field group prefix in registration form','vibebp').' : '.esc_html( apply_filters( 'bp_get_the_profile_group_name', $group->name ) ).'</h4>';
	...
	function show_balance_list(){
		...
		if(!empty($_GET['action']) && $_GET['action']=='history'){
			...
			if ( ! empty( $user_query->get_results() ) ) {
				echo '<input class="search_input" type="text" value="'.sanitize_text_field((!empty($_GET['s'])?$_GET['s']:'')).'"/><a class="button search_user" href="'.vibebpaddOrUpdateUrlParam('paged','').'">'._x('Search','','vibebp').'</a>';
