...
if ( ! class_exists( 'VIBE_BP_API_Rest_Members_Controller' ) ) {
	...
	class Vibe_BP_API_Rest_Members_Controller extends WP_REST_Controller{
		...
		public function register_routes() {
			...
			register_rest_route( $this->namespace, '/members/assign-terms', array(
				array(
					'methods'             => 'POST',
			...
			register_rest_route( $this->namespace, '/members/assign-member-type', array(
				array(
					'methods'             => 'POST',
					'callback'            =>  array( $this, 'assign_member_type' ),
					'permission_callback' => array( $this, 'all_members_user_permissions_check' ),
				),
			));
			...
			register_rest_route( $this->namespace, '/members/create-tag', array(
				array(
					'methods'             => 'POST',
					'callback'            =>  array( $this, 'create_member_tag' ),
					'permission_callback' => array( $this, 'all_members_user_permissions_check' ),
				),
			));
...
*/
    	...
    	function vibe_friends_get_friendId_request_ids_for_user($request){
    		...
    		$args = json_decode($request->get_body(),true);
    		$args = vibebp_recursive_sanitize_text_field($args);
    	    $initiator_friendship_ids=$this->vibe_friends_get_friendship_ids_for_user($this->user->id,intval($args['page']),intval($args['requester']),$args['sort']);
    	    $user_details = array();
		...
		function get_all_members($request){
			...
			if ( ! empty( $user_query->get_results() ) ) {
				foreach ( $user_query->get_results() as $user ) {
					...
					$user->tags = $this->get_usertags($user->ID);
					$user->member_types = $this->get_usermember_types($user->ID);
					$user->ID = (int)$user->ID;
		...
		function get_usertags($user_id){
			$terms = wp_get_object_terms( $user_id, 'member_tag');
			return $terms;
		}

		function get_usermember_types($user_id){
			$terms = wp_get_object_terms( $user_id, 'bp_member_type');
			return $terms;
		}
		...
		function assign_member_terms($request){
			...
			$data = array( 'status' => 0);
			if($body['action'] === 'add_tag' || $body['action'] === 'add_member_type' || $body['action'] === 'remove_tag' || $body['action'] === 'remove_member_type' )
			if(!empty($body['member_ids']) && !empty($body['term_ids']) && !empty($body['action'])){
				$terms = array_map(function ($a) { return (int)$a; }, $body['term_ids']);
				$type = $body['action'] === 'add_member_type' || $body['action'] === 'remove_member_type' ? 'bp_member_type' :'member_tag';
				
				foreach ($body['member_ids'] as $key => $member_id) {
					if($body['action'] === 'add_tag' || $body['action'] === 'add_member_type'){
			...
			if(!empty((int)$body['user_id'])){
				...
				if(bp_is_active('friends')){
					$stats_array[]=array(
						...
						'type' => 'friends_count',
						'label' => __('Firneds Count','members user stats','vibebp'),
		...
		function create_member_tag($request){
			$body = json_decode($request->get_body(),true);
			$body = vibebp_recursive_sanitize_text_field($body );			
			$data = array( 'status' => 0);
			$term_taxonomy = 'member_tag';
			if(!empty($body['tag'])){
				require_once(ABSPATH . 'wp-admin/includes/taxonomy.php'); 
				$tag = $body['tag'];
				if(!term_exists($tag['name'] , $term_taxonomy)){
					$newtag = wp_create_term( $tag['name'], $term_taxonomy );
					if(!empty($newtag['term_id'])){
						$term = get_term($newtag['term_id']);
						if($tag['color']){
							add_term_meta( $newtag['term_id'], 'member-tag-color', $tag['color'], true );
							$term->color= $tag['color'];
						}
						$data = array( 'status' => 1 , 'data' => $term , 'message' =>__('Tag Created!','vibebp'));
					}
				}else{
					$data['message'] = __('Tag already exists!','vibebp');
				}
			}
			return new WP_REST_Response($data, 200);
		}
