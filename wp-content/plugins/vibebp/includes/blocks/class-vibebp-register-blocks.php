...
class VibeBP_Register_Blocks {
				...
				if ( !empty( $group->fields ) ) {
					...
					foreach ( $group->fields as $field ) {
						if(is_object($field)){
							...
							$data = bp_get_profile_field_data(array('field'=>$field->id,'user_id'=>$user_id));
	...
	function memberactions($settings){
		...
		wp_localize_script('vibebp-members-actions','vibebpactions',apply_filters('vibebpactions_translations',array(
       	...
       	$label = '';
       	//if(!empty($settings['post_action_label']) ){$label=$settings['post_action_label'];};

       	return '<span class="profile_data_action">'.$this->get_profile_action($settings['action'],$settings['action_label'],$label).'</span>';
	}
	...
	function get_profile_data($type){
		...
		switch($type){
			...
			break;
			case 'time_in_site':
				return bp_core_time_since(get_userdata($user_id)->user_registered);
			break; 
			case 'last_status_update':
				$activity = bp_get_user_last_activity($user_id);
	...
	function bpavatar($settings){
        ...
        if(bp_is_active('members','cover_image')){
        	...
        	if(!empty($cover_src)){ 
        		...
        		$cover_style = '<style>.background_cover{background:#fafafa; background-size:cover;border-radius:10px;margin:0 -1.5rem 1.5rem;padding:4.5rem 1.5rem 1.5rem;}</style>';
        	}
        }
        return '<a href="'.bp_core_get_user_domain( $user_id ).'"><img src="'.$src.'" '.(empty($style)?'':'style="'.$style.'"').' /></a>'.$cover_style;
	}
