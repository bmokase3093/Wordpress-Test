...
function vibebp_delete_user_active_tokens($user_id,$_token,$tokens=null){
	if(!empty($user_id)){
		if(empty($tokens)){
			$tokens = get_user_meta($user_id,'vibebp_active_tokens',true);
		}
		if(!empty($tokens)){
			foreach ($tokens as $key => $token) {
				...
				if($key==$_token && !empty($tokens[$key])){
					unset($tokens[$key]);
				}
			}
			update_user_meta($user_id,'vibebp_active_tokens',$tokens);
...
function vibebp_featured_style($post,$style,$swiper_id=null){
	...
	switch($style){
		...
		case 'group_block':
			...
			$group_status = vibebp_get_group_statuses();

			?>
			<div class="member_featured_block_wrapper default_member_block member_type_<?php echo $group_type.' group_'.$post->id; ?>" >
				<a href="<?php echo $link;?>">
					<div class="member_background" style="background-image:url(<?php echo $cover_url; ?>)">
						<img src="<?php echo $avatar; ?>" class="member_avatar" />
					</div>
				</a>
				<span><?php echo $group_status[$post->status] ?></span>
				<a href="<?php echo $link;?>"><strong><?php echo esc_attr( $name ); ?></strong></a>
				<?php echo empty($types[$group_type])?'':'<span>'.$types[$group_type]->labels['singular_name'].'</span>'; ?>
				</a>
			</div>
			<?php
		break;
...
function vibebp_generate_carousel_shortcode($settings){
		...
		if(empty($flag)){
		    ...
		    columns='".(empty($settings['carousel_columns'])?3:$settings['carousel_columns'])."' 
		    vertical='".(empty($settings['vertical'])?'':$settings['vertical'])."' 
		    starting_slide='".(empty($settings['starting_slide'])?1:$settings['starting_slide'])."' 
...
function vibebp_groups_get_groupmeta_value($field,$group_id){
	$value = groups_get_groupmeta($group_id,$field['id']);
	switch($field['type']){
		case 'taxonomy':
