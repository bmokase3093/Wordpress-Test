 ...
 class WPLMS_Course_Directory extends \Elementor\Widget_Base  // We'll use this just to avoid function name conflicts 
{
    ...
    public function __construct($data = [], $args = null) {
	    ...
	    foreach($this->get_course_directory_scripts($this->settings) as $k => $script){
	    	$deps = array_merge($script['deps'],array('elementor-frontend'));
	    	wp_register_script($script['id'],$script['src'],$deps , WPLMS_PLUGIN_VERSION, true );
		}
		foreach($this->get_course_directory_styles($this->settings) as $k => $style){
	...
	protected function render() {
		...
		if($settings['show_course_popup']){
	        ...
	        $init2 = WPLMS_Course_Component_Init::init();
	        ...
	        <script>
	            ...
	            var wplms_course_data = <?php echo json_encode($init2->get_wplms_course_data()) ?>;
		...
		<div id="wplms_courses_directory" <?php  echo isset($settings['show_map'])?'class="with_map"':''?>>
			...
			<div class="wplms_courses_directory_main">
				...
				<?php
					if(!empty($settings['search_courses'])){
						?>
		...
		<script>
			var course_directory= <?php echo json_encode($this->args);?>;
			document.dispatchEvent(new Event('course_directory_loaded'));
		</script>
		<?php
		global $post;
	...
	function get_course_directory_scripts($settings=null){
		...
		$scripts =  array(
			...
			array(
				'src'=>plugins_url('../../../../../assets/js/course_directory.js',__FILE__),
				'id'=>'wplms-course-directory-js',
				'deps'=>array('wp-element','wp-data'),
			),
			
		);

		if(!empty($settings['show_course_popup'])){
			$scripts[] = array(
				'id'=>'singlecourse',
				'src'=>plugins_url('../../../../../assets/js/singlecourse.js',__FILE__),
				'deps'=>array('wp-element','wp-data')
			);
			
		}

		return apply_filters('vibebp_course_directory_scripts',$scripts);
	}

		

	function get_course_directory_styles($settings=null){
		$styles =  array(
			array(
			...
			array(
				'src'=>plugins_url('../../../../../assets/css/course_directory.css',__FILE__),
				'id'=>'course-directory',
				'deps'=>array(),
			),
		);

		return apply_filters('vibebp_course_directory_styles',$styles);
	}
}