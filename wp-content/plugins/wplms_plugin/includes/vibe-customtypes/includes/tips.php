...
class WPLMS_tips{
				...
				switch($key){
					...
					case 'course_curriculum_below_description':
						if(function_exists('is_wplms_4_0') && !is_wplms_4_0('course')){
							$id= get_the_ID();
							if(empty(get_post_meta($id,'vibe_course_package',true))){
					...
					break;
					case 'default_my_courses_orderby':
						add_filter('wplms_mycourses',array($this,'wplms_mycourses'));
					break;
					...
					case 'enable_forum_privacy':
						...
						add_filter('VibeBbp_get_forums_api',array($this,'check_helpdesk_privacy'),10,2);
					break;
					case 'gamification':
						add_action('init',function(){
							register_taxonomy( 'lmsbadge-type', array( 'lmsbadge'),
								array(
									'labels' => array(
										'name' => __('Badge Type','wplms'),
										'menu_name' => __('Badge Type','wplms'),
										'singular_name' => __('Badge Type','wplms'),
										'add_new_item' => __('Add New Badge Type','wplms'),
										'all_items' => __('All Badge Types','wplms')
									),
									'public' => true,
									'hierarchical' => true,
									'show_ui' => true,
									...
									'query_var' => 'lmsbadge-type',           
									'show_in_nav_menus' => true,
								)
							);

							register_post_type( 'lmsbadge',
								array(
									'labels' => array(
										'name' => __('Badges','wplms'),
										'menu_name' => __('Badges','wplms'),
										'singular_name' => __('Badge','wplms'),
										'add_new_item' => __('Add New Badge','wplms'),
										'all_items' => __('All Badges','wplms')
									),
									'public' => true,
									...
									'taxonomies' => array( 'lmsbadge-type'),
									'supports' => array( 'title','editor','thumbnail','author','comments','custom-fields', 'page-attributes'),
									'hierarchical' => true,
								)
							);
							...
							$metabox_settings = array(
								'points'=>array( 
									'label'	=> __('Badge Points','wplms'),
									'desc'	=> __('Site wide points at which this badge is awarded to the student.','wplms'),
									'id'	=>'points',
									'type'	=> 'number',
									'std'   => ''
								),
								'subtitle'=>array( 
									'label'	=> __('Badge Sub-Title','wplms'),
									'desc'	=> __('Displays below the badge Image on badge archive page.','wplms'), 
									'id'	=>'subtitle',
									'type'	=> 'textarea',
									'std'   => ''
								) 
							);
							$course_box = new custom_add_meta_box( 'page-settings', __('Badge Settings','wplms'), $metabox_settings, 'lmsbadge', true );
							add_action( 'admin_menu',function(){
								add_submenu_page(
									'lms',
									__('Badge Types','wplms'),
									__('Badge Types','wplms'),
									'manage_options',
									'edit-tags.php?post_type=lmsbadge&taxonomy=lmsbadge-type'
								);
							}, 999 );

							add_action( 'lmsbadge-type_add_form_fields', array( $this, 'add_lmsbadge_type_fields' ));
							add_action( 'lmsbadge-type_edit_form_fields', array( $this, 'edit_lmsbadge_type_fields' ));
							add_action( 'created_term', array($this,'save_lmsbadge_type_meta'), 10, 2 );
							add_action( 'edited_term', array($this,'save_lmsbadge_type_meta'), 10, 2 );
							//upload box in lmsbadge-type
						});
					break;
    ...
    */
	...
	function check_helpdesk_privacy($return,$args){
		if(!empty($args['token']) && !empty($return['forums'])){
			$user = apply_filters('vibebp_api_get_user_from_token','',$args['token']);
			if(!empty($user)){
				$new_forums = [];
				foreach($return['forums'] as $k => $forum){
					if(bpp_can_user_view_post($forum['id'],$user->id)){
						$new_forums[] = $forum;
					}
				}
				$return['forums'] = $new_forums;
			}
		}
		return $return;
	}

	function bpp_can_user_view_post($post_id,$user_id=null){
		global $wpdb;
	...
	function wplms_mycourses($args){
		if(!empty($this->settings['default_my_courses_orderby'])){
			switch ($this->settings['default_my_courses_orderby']) {
				case 'date':
					$args['order']=(!empty($this->settings['default_my_courses_orderby'])?$this->settings['default_my_courses_orderby']:'DESC');
					break;
				case 'title':
					$args['orderby']='title';
					$args['order']=(!empty($this->settings['default_my_courses_orderby'])?$this->settings['default_my_courses_orderby']:'ASC');
					break;
				case 'start_date':
					$args['orderby']=array('menu_order' => (!empty($this->settings['default_my_courses_orderby'])?$this->settings['default_my_courses_orderby']:'DESC'), 'meta_value' => 'ASC');
	...
	function login_redirect_v4_2($r,$expanded_token=null){
		...
		if(!empty($expanded_token) && !empty($expanded_token->data->user)){
			...
			if(!empty($this->settings[$role.'_login_redirect']) && !empty($redirect_array[$this->settings[$role.'_login_redirect']])){
	...
	*/
	...
	function save_lmsbadge_type_meta( $term_id, $tt_id ){
		global $wpdb;
	    if( isset( $_POST['lmsbadge_type_thumbnail_id'] )){
	        $thumb_id = intval( $_POST['lmsbadge_type_thumbnail_id'] );
	        update_term_meta( $term_id, 'lmsbadge_type_thumbnail_id', $thumb_id );
	    }
	    if( isset( $_POST['lmsbadge_type_order'] ) &&is_numeric($_POST['lmsbadge_type_order'])){
	        update_term_meta( $term_id, 'lmsbadge_type_order', $_POST['lmsbadge_type_order'] );
	        $wpdb->update($wpdb->terms, array('term_group' => $_POST['lmsbadge_type_order']), array('term_id'=>$term_id));
	    }
	}
    function add_lmsbadge_type_fields(){
    	...
    	<input type="number" name="lmsbadge_type_order" id="lmsbadge_type_order" value="" />
    	</div>
    	<div class="form-field">
			<label><?php _e( 'Thumbnail', 'wplms' ); ?></label>
			<div id="lmsbadge_type_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $default ); ?>" width="60px" height="60px" /></div>
			<div style="line-height: 60px;">
				<input type="hidden" id="lmsbadge_type_thumbnail_id" name="lmsbadge_type_thumbnail_id" />
			...
			<script type="text/javascript">
				if ( ! jQuery( '#lmsbadge_type_thumbnail_id' ).val() ) {
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame.on( 'select', function() {
						...
						jQuery( '#lmsbadge_type_thumbnail_id' ).val( attachment.id );
						let url_image ='';
						...
						jQuery( '#lmsbadge_type_thumbnail' ).find( 'img' ).attr( 'src', url_image );
				...
				jQuery( document ).on( 'click', '.remove_image_button', function() {
					jQuery( '#lmsbadge_type_thumbnail' ).find( 'img' ).attr( 'src', '<?php echo esc_js( $default ); ?>' );
					jQuery( '#lmsbadge_type_thumbnail_id' ).val( '' );
					jQuery( '.remove_image_button' ).hide();
					return false;
				});
    ...
    */
    function edit_lmsbadge_type_fields($term){


    	$thumbnail_id = absint( get_term_meta( $term->term_id, 'lmsbadge_type_thumbnail_id', true ) );
    	$order = get_term_meta( $term->term_id, 'lmsbadge_type_order', true ); 
		if ( $thumbnail_id ) {
    	...
    	<tr class="form-field">
			...
			<td><input type="number" name="lmsbadge_type_order" id="lmsbadge_type_order" value="<?php echo (empty($order)?0:$order); ?>" /></td>
    	</tr>
    	<tr class="form-field">
			<th scope="row" valign="top"><label><?php _e( 'Thumbnail', 'wplms' ); ?></label></th>
			<td>
				<div id="lmsbadge_type_thumbnail" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $image ); ?>" width="60px" height="60px" /></div>
				<div style="line-height: 60px;">
					<input type="hidden" id="lmsbadge_type_thumbnail_id" name="lmsbadge_type_thumbnail_id" value="<?php echo vibe_sanitizer($thumbnail_id,'text'); ?>" />
					<button type="button" class="upload_image_button button"><?php _e( 'Upload/Add image', 'wplms' ); ?></button>
					<button type="button" class="remove_image_button button"><?php _e( 'Remove image', 'wplms' ); ?></button>
				</div>
				<script type="text/javascript">
					...
					jQuery( document ).on( 'click', '.remove_image_button', function() {
						jQuery( '#lmsbadge_type_thumbnail' ).find( 'img' ).attr( 'src', '<?php echo esc_js( $image ); ?>' );
						jQuery( '#lmsbadge_type_thumbnail_id' ).val( '' );
						jQuery( '.remove_image_button' ).hide();
						return false;
					});
