...
class WPLMS_Plugin_Filters{
	...
	function wplms_get_field_value_mycred($field,$id,$user_id){
		if(!empty($field['id']) && $field['id']=='vibe_mycred_duration'){
			$meta = 9999;
			$parameter = 86400;
    		if(!empty($id)){
    ...
    function wplms_instructor_privacy_filter_attachments($wp_query){
    	...
    	if(function_exists('vibe_get_option')){
    		...
    		if(is_user_logged_in() && !empty($instructor_privacy) && !current_user_can('manage_options')){
    			if (!empty($wp_query->query['post_type']) && $wp_query->query['post_type'] == 'attachment') {
    ...
    function course_cat_orderby($orderby,$args,$taxonomies){
    	if ( is_admin() || (!empty($taxonomies) && 'course-cat' != $taxonomies[0] || !empty($orderby)))
        	return $orderby;
