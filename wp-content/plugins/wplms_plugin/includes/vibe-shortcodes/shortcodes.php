...
Class Vibe_Define_Shortcodes{
    ...
    function __construct(){
       ...
       //course template shortcodes:
        ...
        add_shortcode('course_instructor_card',array($this,'course_instructor_card'));
        ...
        add_shortcode('related_courses',[$this,'related_courses']);
    }
    ...
    function course_curriculum($atts,$content=null){
        ...
        $id = apply_filters('elementor_course_block_id',$id);
        if(empty($id))
            return;
        ...
        switch($info){
            ...
            case 'course-cat':
                ...
                if ( ! empty( $post_terms ) && ! is_wp_error( $post_terms ) ) {
                    ...
                    return  '<div class="wplms_course_categories">'.$terms.'</div><style>
    ...
    function user_reviews($atts,$content=null){
        ...
        if($user_type == 'instructor'){
            ...
            $elements = [];
            if(!empty($hide_id_none) && empty($courses))
                return;
            if(empty($courses)){
    ...
    function course_instructor_card($atts,$content=null){
        ...
        if(!empty($instructors) ){
            if(is_array($instructors)){
                $blog_id = get_current_blog_id();
                foreach($instructors as $k=>$inst){
                  
                    $api_url = apply_filters('vibebp_rest_api',get_rest_url($blog_id,Vibe_BP_API_NAMESPACE));
                    $apipost = wp_remote_get($api_url.'/member_card_post/'.$inst.'/'.$card_id.'?client_id='.vibebp_get_setting('client_id'),array(
                            'timeout'     => 200,
                    ));
                    //render the card here
                
                    $poste = wp_remote_retrieve_body( $apipost );
       
                    if(!empty($poste) && !is_wp_error($poste)){
                        $apiBody = json_decode($poste);
                        if(!empty($apiBody) && !is_wp_error($apiBody) && !is_object($apiBody)){
                            //print_R($apiBody);
                            $html .= $apiBody;
                        }
                    } 
                ...
                $api_url = apply_filters('vibebp_rest_api',get_rest_url($blog_id,Vibe_BP_API_NAMESPACE));
                $apipost = wp_remote_get($api_url.'/member_card_post/'.$instructors.'/'.$card_id.'?client_id='.vibebp_get_setting('client_id'),array(
                        'timeout'     => 200,
                ));
                //render the card here
    ...
    function course_instructor_data($atts,$content=null){
        ...
        if(empty($id))
            return;
        if(empty($this->instructors_data)){
            $aid = get_post_field('post_author',$id);
            $instructors = apply_filters('wplms_course_instructors',array($aid),$id);
            foreach($instructors as $instructor_id){
                $this->instructors_data[$instructor_id] = [];    
            }
            //record this instructor
            $this->instructors_data[$instructors[0]][]=$data;
            $user_id = $instructors[0];
           
        }else{
           
            foreach($this->instructors_data as $instructor_id=>$recorded_fields){
                $user_id = $instructor_id;   
                if(empty($recorded_fields) || (is_array($recorded_fields) && !in_array($data,$recorded_fields))){
                    $this->instructors_data[$instructor_id][] = $data;  
                    $user_id = $instructor_id;     
                    break;
                }
            }
    ...
    function course_featured($atts,$content=null){
        ...
        $html ='<div class="course_featured '.$align.' '.$className.'" style="position:relative;"><span>';
        $img = get_the_post_thumbnail_url($id,'full');
        $html .= '<img src="'.$img.'" class="'.$className.'" style="'.(empty($width)?'':'width:'.$width.'px;').'border-radius:'.$radius.'px;" />';
        $html .='</div>';
        return $html;
    }
    ...
    function course_reviews($atts,$content=null){
        ...
        if(empty($id))
            return;
        $actual_count = $count = get_comments_number($id);
        if($hide_if_none){
        ...
        $args['number'] = $number;
        $args['paged'] = (!empty($_GET['cpaged']))?intval($_GET['cpaged']):1;
        $args['parent'] =0; 
        ...
        ?>
            ...
            <?php 
                  wp_list_comments(array(
                     'type'        =>'comment',
                     'avatar_size' =>120,
                     'callback'    => 'wplms_plugin_course_reviews_with_child',
                     'type'=>'all',
                     ),$comments
                   ); 
              ...
              <div class="review_paginate">
                <?php echo $this->paginate_reviews($args,$actual_count);?>
              </div>  
            </ol> 
        <?php
        $req=1;
        ...
        if(is_user_logged_in() && !empty($_GET['replytocom'])){
            $commenter = get_comment(intval($_GET['replytocom']),ARRAY_A);
            $fields =  array(
           ...
           $comment_field='<p>' . '<textarea id="comment" name="comment" class="form_field" rows="8" "></textarea></p>';
    ...
    function addOrUpdateUrlParam($name, $value,$url)
    {
        ...
        unset($params[$name]);
        $params[$name] = $value;
        return basename($url).'?'.http_build_query($params);
    }

    function paginate_reviews($args,$count){

        $html ='';
        if(!empty($args['number']) && $count>1 && $args['number']<$count){
            if(!empty($args['paged'])){
                $var_name = '?cpaged=';
                global $wp;
                ...
                if($args['paged']>1){
                    $url =$this->addOrUpdateUrlParam('cpaged',($args['paged']-1),$current_url);
                     $html .= '<a href="'.$url.'" class="button vicon vicon-angle-left"></a>';
                }else{
                    $html .= '<a class="button vicon vicon-angle-left"></a>';
                }

                
                if($args['paged']*$args['number'] < $count){
                    $url =$this->addOrUpdateUrlParam('cpaged',($args['paged']+1),$current_url);
                    $html .= '<a href="'.$url.'" class="button vicon vicon-angle-right"></a>';
                }else{
                    $html .= '<a  class="button vicon vicon-angle-right"></a>';
                }
            }
        }
...
/*-----------------------------------------------------------------------------------*/
    ...
    function wplms_registration_form($atts, $content = null){
        ...
        if(function_exists('vibe_get_option') && vibe_get_option('offload_scripts')){
            ...
            wp_localize_script( 'wplms-registration-forms', 'vibe_shortcode_strings', $translation_array );
        }
    ...
    */
    ...
    function related_courses($atts,$content=null){

        $course_id = 0;
        if(!empty($atts['course_id']) && is_numeric($atts['course_id'])){
            $course_id = $atts['course_id'];
        }else{
            $course_id = apply_filters('elementor_course_block_id',$course_id);
            
        }

        if(empty($course_id) || !function_exists('vibebp_generate_carousel_shortcode'))
            return;

        $cats = wp_get_post_terms($course_id,'course-cat',['fields'=>'slugs']);
        ...
        $args = [
            ...
            'post_type'=>'course',
            'carousel_number'=>6,
            'post_type_taxonomy'=>'course-cat',
            'post_type_include_terms'=>implode(',',$cats),
            'carousel_columns'=>4,
            'show_controls'=>1,
