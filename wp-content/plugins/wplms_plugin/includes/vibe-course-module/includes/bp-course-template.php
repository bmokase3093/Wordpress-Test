	...
	if(!function_exists('bp_course_is_member')){
		function bp_course_is_member($course_id = null, $user_id = null){
			...
			if(!is_user_logged_in() && empty($user_id))
		  	...
		  	if(empty($course_id)){
		  		global $post;
		  		if(!empty($post) && $post->post_type == BP_COURSE_CPT){
		  			$course_id = $post->ID;
		  		}else{
		  			return false;
		  		}
		...
		if(!empty($course_items)){
			foreach($course_items as $key => $item){
				if(is_numeric($item)){
					...
					$etype = wplms_get_element_type($item,$type);
					if($type=='quiz' && $etype=='scorm'){
						$duration = '';
					}
					$curriculum[] = array(
						'id'		=>  $item,
						'key'		=>	$key,
						'type'		=>	$type,
						'icon'		=>  wplms_get_element_icon($etype),
...
function bp_course_get_question_details($question_id,$force = null){
	...
	$hint = get_post_meta($question_id,'vibe_question_hint',true);
	if(!empty($hint)){
		$hint_text = strip_tags($hint);
		if(!empty($hint_text)){
			$question['hint'] = do_shortcode($hint);
		}
	}
	...
	$content = apply_filters('vibebp_the_content',$content,$question_id);
	...
	if( has_shortcode( $content, 'match' ) ) {
		...
		$content = preg_replace("/\[fillblank.*?\]/", '||fillblank||', $content);
		$content = apply_filters('the_content',$content);
		$content_array = preg_split("/\|\|fillblank\|\|/", $content);
		...
		$content = preg_replace("/\[select.*?\]/", '||select||', $content);
		$content = apply_filters('the_content',$content);
	...
	$question['content'] = apply_filters('vibebp_the_content',$question['content'],$question_id);	
	...
	if(!empty($question['options'])){
		foreach ($question['options'] as $key => $op) {
			$options_with_the_content[]= apply_filters('vibebp_the_content',do_shortcode($op),$question_id);
		}
	}
	...
	return apply_filters('bp_course_get_question_details',$question,$question_id,$force);
}
...
function bp_course_unit_quiz_complete_check($unit_id,$user_id){
	if(empty($unit_id))
		return false;
...
function bp_get_course_check_course_complete($args=NULL){ // AUTO EVALUATION FOR COURSE
	...
	$course_curriculum=bp_course_get_curriculum_units($id,$user_id);
	if(isset($course_curriculum) && count($course_curriculum)){
		...
		foreach($course_curriculum as $unit_id){
			...
			if(empty($unittaken) && in_array(bp_course_get_post_type($unit_id),array('quiz'))){
		...
		$flag = apply_filters('wplms_finish_course_check',$flag,$course_curriculum,$user_id,$id);
			...
			if(vibe_validate($auto_eval)){
				...
				foreach($curriculum as $c){
					if(bp_course_get_post_type($c) == 'quiz'){
	          			...
	          			$k = intval($k);
						$student_marks += apply_filters('wplms_course_quiz_marks_weightage',$k,$c,$course_id);
...
function bp_course_get_drip_status($course_id,$user_id,$unit_id){
    ...
    if($drip_enable){
    	$completed = bp_course_check_unit_complete($unit_id,$user_id,$course_id);
        if(!empty($completed))
        	return apply_filters('wplms_check_drip_status_unit_complete',array('status'=>0,'message'=>''),$unit_id,$user_id,$course_id);
        ...
        if(!isset($pre_unit_key)){$pre_unit_key = 0;}
