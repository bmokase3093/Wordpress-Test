...
class BP_Course_Action{
    ...
    private function __construct(){
        ...
        add_action('wplms_4_after_the_course_button',array($this,'wplms_4_the_credits_button'));

        add_action('wplms_single_course_content_end',array($this,'wplms_get_course_schema_details'));
	}
     ...
     function wplms_get_course_schema_details(){
        ...
        if(empty($meta) || false === $meta){
            $meta = wplms_get_course_schema_details($id);
    ...
    function wplms_4_the_credits_button($course_id){
        if((is_singular('course') || is_singular('course-layout')) && apply_filters('wplms_4_the_credits_button',true,$course_id)){
            $meta = get_post_meta($course_id,'vibe_course_credits',true);
       
            if(!empty($meta)){
                //echo '<div class="course_credit_button" data-id="'.$course_id.'"></div>';
                ...
                $translation_array = array( 
                    ...
                    'translations'=>array(
                        'ok' => __( 'Ok ','wplms' ), 
                        'credits'=>_x('Credits','','wplms'),
                        'buy_with_credits'=>_x('Buy with Credits','','wplms'),
                    ),
                  
                  
                );
                wp_enqueue_script('vibe-course-credit',plugins_url('../../../assets/js/course_credits.js',__FILE__),array('wp-element'),WPLMS_PLUGIN_VERSION,true);
                wp_localize_script( 'vibe-course-credit', 'vibe_course_credits', $translation_array );
    ...
    function bp_course_admin_search_course_students($students,$course_id){
        ...
        echo '<form id="course_user_ajax_search_results" data-id="'.$course_id.'">
                <select id="active_status"><option value="">'.__('Filter by Status','wplms').'</option>';
                foreach($active_statuses as $key =>$value){
                    echo '<option value="'.$key.'">'.$value.'</option>';
                }
        echo  '</select>
                <select id="course_status"><option value="">'.__('Filter by Status','wplms').'</option>';
                foreach($course_statuses as $key =>$value){
                    echo '<option value="'.$key.'">'.$value.'</option>';
                }
        echo  '</select>';
        do_action('wplms_course_admin_form',$students,$course_id);
        echo '<span id="search_course_member"><input type="text" name="search" placeholder="'.__('Enter student name/email','wplms').'" /></span>
             </form>';
	...
	function bp_course_enable_access($order_id){
		...
		$order = new WC_Order( $order_id );
		$items = $order->get_items();
		$user_id= apply_filters('wplms_order_user_id',$order->get_user_id(),$order_id);
		$order_total = $order->get_total();
		...
		$instructor_commission = null;
        if(function_exists('vibe_get_option')){
          $instructor_commission = vibe_get_option('instructor_commission');
          if($instructor_commission === 0 || $instructor_commission === '0')
                return;
        }
	...
	function set_dynamic_question_set($quiz_id=NULL,$user_id=NULL,$bypass=NULL){
	    ...
	    if($quiz_dynamic=='dynamic'){ 
            ...
            if(isset($quiz_questions) && $quiz_questions && is_array($quiz_questions) && count($quiz_questions['ques'])){
                return;
            }
