...
$course_admin_tabs = apply_filters('wplms_course_admin_tabs',array(
	'admin'=> array('name'=> _x('Members','Course admin tab name','wplms'),'url'=>get_permalink(get_the_ID()).apply_filters('wplms_course_admin_slugs','?action=admin')),
	'submissions'=>array('name'=> _x('Submissions','Course admin tab name','wplms'),'url'=>get_permalink(get_the_ID()).apply_filters('wplms_course_admin_slugs','?action=admin&submissions')),
	'stats'=>array('name'=> _x('Stats','Course admin tab name','wplms'),'url'=>get_permalink(get_the_ID()).apply_filters('wplms_course_admin_slugs','?action=admin&stats')),
	));
?>
...
?>	
	<h4 class="total_students"><?php _e('Total number of Students in course','wplms'); ?><span><?php echo $students; ?></span></h4>
	<?php
	...
	if(count($students_undertaking)>0){
			...
			if(function_exists('bp_course_admin_userview')){
				...
				if (function_exists('bp_get_profile_field_data')) {
				    ...
				    if ($bp_name) {
					    ...
					    echo '<ul> 
					    		<li><a class="tip reset_course_user" data-course="'.get_the_ID().'" data-user="'.$student.'" title="'.__('Reset Course for User','wplms').'"><i class="icon-reload"></i></a></li>
					    		<li><a class="tip course_stats_user" data-course="'.get_the_ID().'" data-user="'.$student.'" title="'.__('See Course stats for User','wplms').'"><i class="icon-bars"></i></a></li>';
			    		...
			    		echo '<li><a href="'.get_permalink().$activity_slug.'student='.$student.'" class="tip" title="'.__('See User Activity in Course','wplms').'"><i class="icon-atom"></i></a></li>
					    		<li><a class="tip remove_user_course" data-course="'.get_the_ID().'" data-user="'.$student.'" title="'.__('Remove User from this Course','wplms').'"><i class="icon-x"></i></a></li>
		...
		echo '<div id="message" class="notice"><p>'.__('No Students found.','wplms').'</p></div>';
	}
	if(count($students_undertaking)>0){

		echo '<li><div class="pagination"><div><div class="pag-count" id="course-member-count">'.sprintf(__('Viewing page %d of %d ','wplms'),1,$max_page).'</div>';
		echo '<div class="pagination-links"><span class="page-numbers current">'._x('1','pagination number course admin','wplms').'</span>';
		$f=$g=1;
		if($max_page > 1){
	...
	echo '<div class="course_bulk_actions">
			<strong>'.__('BULK ACTIONS','wplms').'</strong> ';
