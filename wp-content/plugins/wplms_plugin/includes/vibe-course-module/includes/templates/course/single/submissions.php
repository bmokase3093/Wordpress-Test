...
_e('ASSIGNMENTS SUBMISSIONS','wplms');
		...
		if(get_post_type($c) == 'quiz'){
			...
			if(count($members_unchecked_quiz)){
				foreach($members_unchecked_quiz as $unchecked_quiz ){
				    ...
				    // PENDING AJAX SUBMISSIONS
				    echo '<ul> 
				    		<li><a class="tip reset_quiz_user" data-quiz="'.$c.'" data-user="'.$member_id.'" title="'.__('Reset Quiz for User','wplms').'"><i class="icon-reload"></i></a></li>
				    		<li><a class="tip evaluate_quiz_user" data-quiz="'.$c.'" data-user="'.$member_id.'" title="'.__('Evaluate Quiz for User','wplms').'"><i class="icon-check-clipboard-1"></i></a></li>
				    	  </ul>';
				    echo '</li>';
					}
				}
