...
if (!class_exists('WPLMS_Assignments')){
    ...
    public static function init(){
        ...
        public function __construct(){ 
            ...
            $this->assignment_submissions = array();
        ...
        function evaluate_assignment(){
           ...
           if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'vibe_assignment') ){
            ...
            echo '<h3 class="heading">'.__('SUBMISSION','wplms').'</h3>';
                ...
                if(!empty($attachment_id)){
                  if(is_array($attachment_id)){
                    foreach($attachment_id as $attachid){
                      ...
                      echo '<div class="download_attachment"><a href="'.$url.'" target="_blank" download="'.filename($url).'"><i class="icon-download-3"></i> '.__('Download Attachment','wplms').'</a></div>';
                    }
                  }else{
            ...
            $user_marks = get_post_meta($assignment_id,$user_id,true);
            ...
            if(empty($course_id)){
              ...
              $remarkmessage = $wpdb->get_results($wpdb->prepare( "
                                        ...
                                        AND     activity.secondary_item_id = %d
                                        ORDER BY activity.date_recorded DESC LIMIT 0,1
                                    " ,$course_id,$instructor_id,$assignment_id));
              
            }

            if(!empty($remarkmessage)){
              $remarkmessageid = $remarkmessage[0]->meta_value;
              
            }
            
            echo '<h4>'.sprintf(_x('%s submission by %s','Assignment submission by User','wplms'),get_the_title($assignment_id),bp_core_get_user_displayname($user_id)).'</h4>';
            echo '<div class="marks_form">
            <input type="number" class="form_field" id="assignment_marks" placeholder="'.__('Enter marks out of ','wplms').$max_marks.'" value="'.$user_marks.'"/>
            <label>'.__('REMARKS (if any)','wplms').'</label>';
            if(!empty($remarkmessageid)){
                echo '<br><a href="'.trailingslashit( bp_loggedin_user_domain() . $bp->messages->slug . '/view/' . $remarkmessageid ).'" class="button small">'.__('See Old Remarks','wplms').'</a><span class="clearfix"></span>';
            }
            echo '<textarea id="remarks_message"></textarea>';
 
            ?>
            <!--plupload form here -->
            <label><?php echo __('Attach File','wplms')?></label>
            <?php
            $required = ATT_REQ ? ' <span class="required">*</span>' : '';
            echo '<p class="comment-form-url comment-form-attachment">'.
                    '<label for="attachment"><small class="attachmentRules">&nbsp;&nbsp;('.__('Allowed file types','wplms').': <strong>'. $this->displayAllowedFileTypes($assignment_id) .'</strong>, '.__('maximum file size','wplms').': <strong>'. $this->getmaxium_upload_file_size($assignment_id) .__('MB(s)','wplms').'.</strong></small></label>'.
                '</p>';
            ?>
              ...
              <div id="plupload-upload-ui" class="hide-if-no-js">
                  <div id="drag-drop-area">
                      <div class="drag-drop-inside">
                          <p class="drag-drop-info"><?php _e('Drop files here','wplms'); ?></p>
                          <p><?php _ex('or', 'Uploader: Drop files here - or - Select Files','wplms'); ?></p>
                          <p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php _e('Select Files','wplms'); ?>" class="button" /></p>
                      </div>
                  </div>
              </div>
                  ...
                  uploader.bind('FilesAdded', function(up, files){
                      ...
                      plupload.each(files, function(file){
                        if (file.name.match(/\s/g)){
                            alert('<?php echo __('There is a space in file name .Please remove space from your filename and try to re-upload','wplms');?>');
                            return false;
                        }
                         ...
                         /* $.ajax({
                              ...
                              success: function (html) {
                                  if(html == '1'){
                                      ...
                                      up.stop();
                                      up.refresh();
                                      $('.pl_assignment_progress div.'+file.id).html("<div class='message animate tada load has-error text-danger file_exists'><b>("+file.name+")</b><?php echo __('  already exists. Please rename the file and try again.','wplms') ?></div>");
                         ...
                         /* $('.stop_s3_plupload_upload').on('click',function() {
                              if(confirm("<?php echo __('Are you sure you want to quit uploading?','wplms');?>")){
            ...
            <?php
            echo '<a id="give_assignment_marks" class="button small" data-ans-id="'.$answer->comment_ID.'">'.__('GIVE MARKS','wplms').'</a></div>';
            die();
        }
        ...
        */
                ...
                foreach($files as $file){
                    if($file['size'] > 0){
                        ...
                        if(in_array($fileType, $this->getAllowedMimeTypes($savedassignment->comment_post_ID)) && in_array(strtoupper($fileExtension), $this->getAllowedFileExtensions($savedassignment->comment_post_ID)) ){
                          ...
                          unset($_FILES);
                          wp_die('<strong>'.__('ERROR:','wplms').'</strong> '.__('File you upload must be valid file type','wplms').' <strong>('. $this->displayAllowedFileTypes() .')</strong>'.__(', and under ','wplms'). $this->getmaxium_upload_file_size($savedassignment->comment_post_ID) .__('MB(s)!','wplms'));
                        }
        ...
        function insert_assignment_attachment(){
            if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'wplms_assignment_plupload_final') || !is_user_logged_in()){
                ...
                $error =__('Security check failed contact administrator','wplms');
                wp_die($error);
            }else{
                if(!empty($_POST['user'])){
                  $user_id =  $_POST['user'];
        ...
        public function displayFormAtt(){  
            ...
            $required = ATT_REQ ? ' <span class="required">*</span>' : '';
            echo '<p class="comment-form-url comment-form-attachment">'.
                    '<label for="attachment">' . ATT_TITLE . $required .'<small class="attachmentRules">&nbsp;&nbsp;('.__('Allowed file types','wplms').': <strong>'. $this->displayAllowedFileTypes() .'</strong>, '.__('maximum file size','wplms').': <strong>'. $this->getmaxium_upload_file_size() .__('MB(s)','wplms').'.</strong></small></label>'.
                '</p>';

            if(isset($answers) && is_array($answers) && count($answers)){
                    $attachmentIDs = get_comment_meta($answer->comment_ID,'attachmentId',true);
                    echo '<p class="assignment_submission"><strong>'.__('RECENTLY UPLOADED','wplms').' : </strong>';
                ...
                if($this->checkformagain == 0){
                    ...
                    ?>
                            ...
                            <script type="text/javascript">
                                ...
                                jQuery(document).ready(function($){
                                    $( 'body' ).undelegate( '.delete_attachment', "click");
