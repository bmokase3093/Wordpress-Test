...
function assignment_comment_handle($comment, $args, $depth){
    ...
    ?>
        ...
        <?php printf('<span>'.__('Submitted by','wplms').'</span> <cite class="fn">%s</cite>', get_comment_author_link()) ?>
        </div>
    <?php if ($comment->comment_approved == '0') : ?>
            <em class="comment-awaiting-moderation"><?php _e('Assignment awaiting moderation.','wplms') ?></em>
            <br />
    <?php endif; ?>
        ...
        <div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
            <?php
                ...
                printf( __('%1$s at %2$s','wplms'), get_comment_date(),  get_comment_time()) ?></a><?php edit_comment_link(__('(Edit)','wplms'),'  ','' );
            ?>
        </div>
    ...
    global $post;
      ...
      if($remaining > 86400){
        ...
        echo '<div class="assignment_timer '.(($start)?'start':'').'" data-time="'.$remaining.'">
          ...
          <span>'.__('Time Remaining','wplms').'</span>
          <span><strong>'.__('Hours','wplms').'</strong> '.__('Minutes','wplms').'</span>
          </div>';
      }
  }
}
...
function assignment_start_button(){ // Check on Start Values
    ...
    if($flag){
        echo '<form method="post">
              <input type="submit" name="start_assignment" class="button primary full" value="'.__('START ASSIGNMENT','wplms').'" />';
...
function assignment_continue_button(){ // Check on Start Values
    ...
    if(!is_user_logged_in())
        ...
        echo '<form method="post">
              <input type="submit" name="continue_assignment" class="button primary full" value="'.__('CONTINUE ASSIGNMENT','wplms').'" />';
              wp_nonce_field('assignment'.$post->ID,'security');
        echo '</form>'; 
}
    ...
    if(isset($evaluation)){
        ...
        if(isset($coursestarted) && $coursestarted > time()){ // Course is Still active
          if (wplms_assignment_answer_posted()){ 
            echo '<form method="post">
                  <input type="submit" name="submit_assignment" class="submit_assignment button primary" value="'.__('SUBMIT ASSIGNMENT','wplms').'" />';
                  wp_nonce_field('assignment'.$post->ID,'security');
            echo '</form>'; 
          }
        }else{
            echo '<a href="'.get_permalink($course).'" class="button primary full">'.__('INACTIVE COURSE ','wplms').'</a>';
        }
    }else{
       if (wplms_assignment_answer_posted()){
          echo '<form method="post">
              <input type="submit" name="submit_assignment" class="button primary full" value="'.__('SUBMIT ASSIGNMENT','wplms').'" />';
              wp_nonce_field('assignment'.$post->ID,'security');
          echo '</form>'; 
       }
    }
...
function assignment_results_button(){  
    echo '<a href="'.bp_loggedin_user_domain().BP_COURSE_SLUG.'/'.BP_COURSE_RESULTS_SLUG.'/?action='.get_the_ID().'" class="button primary full">'.__('CHECK RESULTS','wplms').'</a>';
}
...
function wplms_assignment_start(){
    ...
    if ( !isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'assignment'.$post->ID ) ) {
        wp_die(__('Security check failed !','wplms'));
       exit;
    }
    ...
    if(isset($_POST['start_assignment'])){
        ...
        if(empty($exists)){
          if(add_user_meta($user_id,$post->ID,time())){
              ...
              wp_die(__('Assignment can not be re-started','wplms'));
          }
    ...
    if(isset($_POST['continue_assignment'])){ //Added Security measure, if someone renames the hidden field and submits 
        ...
        if($time_limit > time())
            ...
            wp_die(__('TIME EXPIRED, PLEASE SUBMIT THE ASSIGNMENT','wplms'));
        }
    ...
    if(isset($_POST['submit_assignment'])){
        if(add_post_meta($post->ID,$user_id,0)){
          ...
          if(function_exists('messages_new_message')){
            $message = __('Assignment ','wplms').get_the_title($post->ID).__(' submitted by student ','wplms').bp_core_get_userlink($user_id);
            messages_new_message( array('sender_id' => $user_id, 'subject' => __('Assignment submitted by Student','wplms'), 'content' => $message,   'recipients' => $post->post_author ) );
          }
...
function wplms_clear_previous_submissions(){
  ...
  if(isset($_POST['security']) && wp_verify_nonce($_POST['security'],'user'.$user_id)){
    ...
    echo $rows.__(' previous submissions removed ! Reloading page ...','wplms');
  }else
...
function wplms_reset_assignment(){
    ...
    if ( !is_numeric($assignment_id) || get_post_type($assignment_id) != 'wplms-assignment'){
        echo '<p>'.__(' Incorrect Assignment','wplms').'</p>';
        die();
    }
      ...
      delete_post_meta($assignment_id,$user_id); // Optional validates that user can retake the quiz
        ...
        if($result)
            echo '<p>'.__('Assignment Reset for Selected User','wplms').'</p>';
        else
            echo '<p>'.__('Could not find Assignment submission for user.','wplms').'</p>';
...
if(is_numeric($answer_id) && is_numeric($value)){
  ...
  if(is_object($comment))
      ...
      $marks = $value;
      $message = sprintf(_x('You\'ve obtained %s out of %s in Assignment : %s Check Results %s. %s Additional Remarks from Instructor %s %s %s','','wplms'),$value,$max_marks,'<a href="'.get_permalink($comment->comment_post_ID).'">'.get_the_title($comment->comment_post_ID).'</a>
      <a href="'.bp_core_get_user_domain( $comment->user_id ).'course/course-results/?action='.$comment->comment_post_ID.'">','</a>',
      '<h3>','</h3>','<br />',$remarks);
      $message_id='';
      if(function_exists('messages_new_message')){
        $message_id=messages_new_message( array('sender_id' => get_current_user_id(), 'subject' => __('Assignment results available','wplms'), 'content' => $message,   'recipients' => $comment->user_id ) );
      }
...
function wplms_assignments_manual_evaluation($course_id,$user_id){
  ...
  foreach($members_assignment_marks as $members_assignment_mark){
       ...
       echo '<li>
          <strong>'.get_the_title($members_assignment_mark['assignment_id']).' <span>'.((isset($members_assignment_mark['marks']) && $members_assignment_mark['marks'] )?'<i class="icon-check"></i> '.$members_assignment_mark['marks'].__(' out of ','wplms').$maximum_marks :'<i class="icon-alarm-1"></i>'.__(' PENDING','wplms')).'</span></strong>
        </li>';
  }
...
function wplms_assignments_stats($list){
  $list['stats_student_assignment_marks']= __('Assignment scores','wplms');
  return $list;
}
